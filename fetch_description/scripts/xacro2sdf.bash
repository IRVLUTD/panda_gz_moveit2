#!/usr/bin/env bash
# This script converts xacro (URDF variant) into SDF for `fetch_description` package

SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" &>/dev/null && pwd)"
XACRO_PATH="$(dirname "${SCRIPT_DIR}")/urdf/fetch.urdf.xacro"
SDF_PATH="$(dirname "${SCRIPT_DIR}")/fetch/model.sdf"
